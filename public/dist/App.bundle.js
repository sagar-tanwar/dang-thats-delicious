...
var autocomplete = function autocomplete(input, lat, lng) {
  if (!input) return;
...
function ajaxHeart(e) {
  e.preventDefault();
  var heartForm = e.currentTarget;
  _axios2.default.post(heartForm.action).then(function (_ref) {
    var data = _ref.data;

    var isHearted = heartForm.heart.classList.toggle('heart__button--hearted');
    if (isHearted) {
      heartForm.heart.classList.add('heart__button--float');
      setTimeout(function () {
        return heartForm.heart.classList.remove('heart__button--float');
      }, 2500);
    }
...
var searchResultHTML = function searchResultHTML(stores) {
  return stores.map(function (store) {
    return '<a class="search__result" href="/store/' + store.slug + '">\n      <strong>' + store.name + '</strong>\n    </a>';
  }).join('');
};
...
function typeAhead(search) {
  ...
  searchInput.on('input', function (event) {
    ...
    _axios2.default.get('/api/search?q=' + event.target.value).then(function (res) {
      if (res.data.length) {
        searchResult.innerHTML = _dompurify2.default.sanitize(searchResultHTML(res.data));
      } else {
        searchResult.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results for ' + event.target.value + ' found!</div>');
      }
    }).catch(function (err) {
      return console.error(err);
    });
  ...
  searchInput.on('keyup', function (e) {
    ...
    var results = searchResult.querySelectorAll('.search__result');
    if (results.length) {
      if (e.keyCode === 40) {
      ...
      results.forEach(function (result) {
        return result.classList.remove('search__result--active');
      });
